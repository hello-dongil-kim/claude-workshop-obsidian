...
> - vault 입구가 여럿인데, **자주 쓰는 입구**는 한 줄 명령으로 자동화 = **수집 스킬**(SKILL.md, 자동 트리거하는 마크다운 설명서).
> - 사례 3개(pdf2md·wiki2md·meeting-log)의 공통 골격 = **입력 → 변환 → frontmatter 붙여 vault 저장.**
> - 핵심 — **직접 코딩 안 한다.** Claude에게 "이런 수집 스킬 만들어줘" 하면 SKILL.md를 짜준다(AI가 내 도구까지 만든다).
> - 최소 골격은 name·description(트리거 키워드)만 갖추면 되고, `templates/collector-skill-template.md`에 그대로 쓸 틀이 있다.
