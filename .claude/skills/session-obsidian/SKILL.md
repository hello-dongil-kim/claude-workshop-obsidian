...
### Phase A (첫 번째 턴)
1. references 파일의 **EXPLAIN**을 읽되, 각 EXPLAIN 상단 **🎙️ 설명 대본(낭독용, ≤6줄)** 을 기준으로 압축 설명한다. reference 본문을 그대로 복붙·낭독하지 말 것 — 표·세부·예시는 학습자가 물을 때만 펼친다.
...
5. **자동 진행(타이핑 제거):** 채점·progress 갱신이 끝나면 확인 없이 곧바로 **같은 턴에서 다음 블록 Phase A**로 이어간다. 학습자가 "다음"을 직접 입력하지 않게 한다.
   - **예외(한 번만 확인):** 다음이 선택 부록(Block 6·7)이거나 마지막 블록 다음이면, 자동 진행 대신 계속할지 한 번 묻는다.
   - 학습자가 언제든 "그만/멈춰/여기까지"라고 하면 즉시 정지한다.
...
- **reference에 없는 단축키를 지어내기** — Obsidian 단축키는 OS(mac/Win)·버전·사용자 재설정에 따라 다르다. 확실한 기본 진입점은 **명령 팔레트**뿐이다. 특정 키가 안 통하면 "명령 팔레트(`Cmd/Ctrl+P`)에서 이름으로 검색" 또는 "설정 → 단축키(Hotkeys)에서 확인"으로 안내하고, reference에 적힌 키만 인용한다
